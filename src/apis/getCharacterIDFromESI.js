...
const getCharacterIDFromESI = async (characterName) => {
  ...
  const response = await get(
    `https://esi.evetech.net/latest/search/?categories=character&datasource=tranquility&language=en-us&search=${characterName}&strict=true`
  );
  ...
  if (characterID == null)
    throw new Error(`Unable to find character info for ${characterName}.`);
