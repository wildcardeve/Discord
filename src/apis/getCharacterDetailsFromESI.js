...
const getCharacterDetailsFromESI = async (characterID, characterName) => {
  ...
  const response = await get(
    `https://esi.evetech.net/latest/characters/${characterID[0]}/?datasource=tranquility`
  );
  ...
  if (characterDetails == null)
    throw new Error(
      `Unable to find character details for character: ${characterName}.`
    );

  logger.info(
    `Found character details for ${characterName}: ${JSON.stringify(
      characterDetails
    )}`
  );
