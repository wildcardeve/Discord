...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("price")
    .setDescription(
      "Provides the price of an item at various EVE Online trade hubs."
    )
    .addStringOption((option) =>
      option
        .setRequired(true)
        .setName("item")
        .setDescription("Item to pricecheck.")
    ),
  async execute(interaction) {
    ...
    const embeddedResponse = new MessageEmbed()
      ...
      .setThumbnail(
        `https://images.evetech.net/types/${item.id}/render?size=128`
      )
      .addFields({ name: "Jita Sell: ", value: priceData.jitaSell.toString() })
      .setTimestamp()
      .setFooter({
