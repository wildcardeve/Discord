...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('who')
    .setDescription('Displays information on the provided EVE Online character.')
    .addStringOption((option) => option.setRequired(true).setName('name').setDescription('EVE Online Character')),
  async execute(interaction) {
    ...
    const characterDetails = await getCharacterDetailsFromESI(characterId, value);
    const killActivity = await getCharacterKillActivityFromZkb(characterId, value);
    const corpDetails = await getCorpDetailsFromESI(characterDetails.corporation_id, value);
    const allianceDetails = await getAllianceDetailsFromESI(characterDetails.alliance_id, value);
    ...
    const embedResponse = new MessageEmbed()
      ...
      .addFields(
        { name: 'Active Recently?', value: `${killActivity.isActive ? 'Yes' : 'No'}` },
        { name: 'Kills: ', value: `${killActivity.shipsDestroyed}`, inline: true },
        { name: 'Losses: ', value: `${killActivity.shipsLost}`, inline: true },
        { name: 'Solo Kills: ', value: `${killActivity.soloKills}`, inline: true },
      )
      .setTimestamp()
      .setFooter({
